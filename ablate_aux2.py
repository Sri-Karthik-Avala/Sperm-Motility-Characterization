...
A2, S2, X2 = build_all(COST)
AUX = np.concatenate([A1, A2], 1)
print("aux dims", A1.shape, A2.shape, AUX.shape)
N = len(tt)
...
np.savez(os.path.join(D, "cache", "oof_aux2.npz"), **oof, fold=fold)
...
print("AUX2-ONLY best decode", best)
p = decode(oof, *best[1:])
