...
OUTD = str(Path(sys.argv[2]).parent) if len(sys.argv) > 2 else "working"
np.savez(os.path.join(OUTD, "dump_oof.npz"), fold=fold, **OOF)
np.savez(os.path.join(OUTD, "dump_test.npz"), **TESTP)
np.save(os.path.join(OUTD, "dump_ftr.npy"), FTR)
np.save(os.path.join(OUTD, "dump_fte.npy"), FTE)
open(os.path.join(OUTD, "dump_fnames.txt"), "w").write(chr(10).join(FNAMES))
...
POPS = [np.ones(NTR, bool), mv, vcl >= med_mv]
